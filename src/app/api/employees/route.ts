...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (employeeId) {
      console.log(`Fetching a single employee with ID: ${employeeId}`);
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!newEmployeeData || !newEmployeeData.employee_id || !newEmployeeData.full_name || !newEmployeeData.email || !newEmployeeData.password) {
